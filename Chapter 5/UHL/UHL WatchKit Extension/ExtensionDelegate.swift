...
final class ExtensionDelegate: NSObject, WKApplicationDelegate {
  func handle(_ backgroundTasks: Set<WKRefreshBackgroundTask>) {
    backgroundTasks.forEach { task in
      ...
      if lastMatchPlayedRecently() {
        ...
        snapshotUserInfo = SnapshotUserInfo(handler: handler, destination: .record)
      } else if let id = idForPendingMatch() {
        print("Going to schedule")
        snapshotUserInfo = SnapshotUserInfo(handler: handler, destination: .schedule, matchId: id)
      }
      if let snapshotUserInfo {
        NotificationCenter.default.post(name: .pushViewForSnapshot, object: nil, userInfo: snapshotUserInfo.encode())
      } else {
        handler()
      }
    }
  ...
  private func nextSnapshotDate() -> Date {
    ...
    let twoDaysLater = Calendar.current.date(byAdding: .day, value: 2, to: nextMatch.date)!
  ...
  private func lastMatchPlayedRecently() -> Bool {
    ...
    return Calendar.current.isDateInYesterday(last) || Calendar.current.isDateInToday(last)
  }
  
  private func idForPendingMatch() -> Match.ID? {
    guard let match = Season.shared.nextMatch else { return nil }
    let date = match.date
