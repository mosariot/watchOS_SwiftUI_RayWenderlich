...
final class Connectivity: NSObject, ObservableObject {
  ...
  public func send(
    ...
    switch delivery {
    case .failable:
      WCSession.default.sendMessage(userInfo, replyHandler: optionalMainQueueDispatch(handler: replyHandler), errorHandler: optionalMainQueueDispatch(handler: errorHandler))
    case .guaranteed:
  ...
  public func send(
    ...
    guard canSendToPeer() else { return }
    WCSession.default.sendMessageData(data, replyHandler: optionalMainQueueDispatch(handler: replyHandler), errorHandler: optionalMainQueueDispatch(handler: errorHandler))
  }
  ...
  public func sendQrCodes(_ data: [String: Any]) {
    ...
    TicketOffice.shared
      ...
      .forEach { movie in
        let image = QRCode.generate(movie: movie, size: .init(width: 100, height: 100))
...
extension Connectivity: WCSessionDelegate {
  ...
  func session(_ session: WCSession, didReceiveMessageData messageData: Data, replyHandler: @escaping (Data) -> Void) {
    // to be provided based on type of data transferring in the app
  }
  ...
  func session(_ session: WCSession, didReceive file: WCSessionFile) {
    let key = ConnectivityUserInfoKey.qrCodes.rawValue
    guard let id = file.metadata?[key] as? Int else { return }
